...
public class JgitRebaser implements Rebaser {
    private static final Logger LOG = LoggerFactory.getLogger(JgitRebaser.class);

    private final JgitGitLib gitLib;


    public JgitRebaser(JgitGitLib gitLib) {
        this.gitLib = gitLib;
    }


    @Override
    public Either<String, SuccessfulRebase> rebase(Branch baseBranch) {
        ...
        return rebaseResult.getStatus().isSuccessful() ?
            right(new JgitSuccessfulRebase(rebaseResult)) :
    ...
    private static class JgitSuccessfulRebase implements SuccessfulRebase {
        private final RebaseResult rebaseResult;


        public JgitSuccessfulRebase(RebaseResult rebaseResult) {
