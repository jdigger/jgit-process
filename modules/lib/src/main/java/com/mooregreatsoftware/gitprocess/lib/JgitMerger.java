...
public class JgitMerger implements Merger {
    private static final Logger LOG = LoggerFactory.getLogger(JgitMerger.class);

    private final JgitGitLib gitLib;


    public JgitMerger(JgitGitLib gitLib) {
        this.gitLib = gitLib;
    }


    @Override
    public Either<String, SuccessfulMerge> merge(Branch mergeBranch) {
        ...
        JgitMerger.LOG.debug("Merging \"{}\"({}) with \"{}\"({})", currentBranch.shortName(), startCurrentOid.abbreviate(7).name(), mergeBranch.shortName(), startIntegrationOid.abbreviate(7).name());
